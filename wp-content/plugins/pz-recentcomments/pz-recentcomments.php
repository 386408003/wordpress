...
Description: Recent comments widget.最新评论
Version: 1.3.0
Author: poporon
...
//				}
				...
				$output		.=	'<span class="comment-content-link"><A href="'.get_comment_link( $comment->comment_ID ).'"><span class="comment-content"'.( $mouseover ? ' title="'.esc_html($comment->comment_content).'"' : '' ).'" '.$color.'>'.get_comment_excerpt( $comment->comment_ID ).'</span></a></span>';
	...
	public function form( $instance ) {
		...
		$color		= isset( $instance['color_guest']	) ? $instance['color_guest'] : '' ;
		$color_user	= isset( $instance['color']			) ? $instance['color'] : '' ;
